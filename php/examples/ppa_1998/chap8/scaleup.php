<?php
/*
 * scaleup.php - corrected Client/server model
 *
...
function main()
{
	...
	for ($dev = 0; $dev < $noNodes; $dev++) {
		if ($node[$dev]->devname == "PC") $uws += $util[$dev];
		if ($node[$dev]->devname == "GW") $ugw += $util[$dev];
		if ($node[$dev]->devname == "FS") $ufs += $util[$dev];
		for ($i = 0; $i < FS_DISKS; $i++) {
			if ($node[$dev]->devname == $FDarray[$i]->label) $udsk[$i] += $util[$dev];
		};
		if ($node[$dev]->devname == "MF") $umf += $util[$dev];
		for ($i = 0; $i < MF_DISKS; $i++) {
			if ($node[$dev]->devname == $MDarray[$i]->label) $udasd[$i] += $util[$dev];
		};
