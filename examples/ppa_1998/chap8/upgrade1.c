...
upgrade1.c - Corrected client/server model for PPA Chap. 8

Upgrade scenario #1 (per the CMG93 paper) includes:
- Add another FS disk
...
char        *scenario = "CMG93 C/S upgrade1";
...
#define		FSD_IOps	(128.9)
#define		FSD_Hit		(0.5)
#define		MF_DISKS	 4
#define		MFD_IOps	(60.24)
...
#define		TR_Bytes	(512 + 0) // different data frame size too
...
main()
{
   ...
   */
   ...
   for (i = 0; i < FS_DISKS; i++) {
       demand[Req_CD][FDarray[i].id] = (1.0 * FSD_Hit / FSD_IOps) / FS_DISKS;
       demand[Req_RQ][FDarray[i].id] = (1.5 * FSD_Hit / FSD_IOps) / FS_DISKS;
       demand[Req_SU][FDarray[i].id] = (0.2 * FSD_Hit / FSD_IOps) / FS_DISKS;
       demand[CD_Msg][FDarray[i].id] = (1.0 * FSD_Hit / FSD_IOps) / FS_DISKS;
       demand[RQ_Msg][FDarray[i].id] = (1.5 * FSD_Hit / FSD_IOps) / FS_DISKS;
       demand[SU_Msg][FDarray[i].id] = (0.5 * FSD_Hit / FSD_IOps) / FS_DISKS;
   }


   /* Mainframe DASD IOs = (#accesses / (max IOs/Sec)) / #disks */
   for (i = 0; i < MF_DISKS; i++) {
       demand[MF_CD][MDarray[i].id] = (2.0 / MFD_IOps) / MF_DISKS;
       demand[MF_RQ][MDarray[i].id] = (4.0 / MFD_IOps) / MF_DISKS;
       demand[MF_SU][MDarray[i].id] = (1.0 / MFD_IOps) / MF_DISKS;
   }
   ...
   */
   ...
   /*
    * Get node total utilizations. This is a bit of a hack and should really be written as
    * a subroutine.
    */
