...
#  abcache.pl
#
# Updated by NJG on Oct 04, 2018
# Use GetNodesCount() and GetStreamsCount()
...
pdq::CreateNode($BUS, $pdq::CEN, $pdq::FCFS);

## Create per CPU cache queueing centers 
for ($i = 0; $i < $MAXCPU; $i++) {
   $cname = sprintf "%s%d", $L2C, $i;
   pdq::CreateNode($cname, $pdq::CEN, $pdq::FCFS);
   #printf "i %2d  cname %10s  nodes %d\n", $i, $cname, pdq::GetNodesCount();
}
...
for ($i = 0; $i < $no; $i++) {
   ...
   pdq::CreateClosed($wname, $pdq::TERM, $Nrwht, $Zrwht);
   $cname   = sprintf "%s%d", $L2C, $i;
   #printf "cname %s\n", $cname;
   ...
   printf "i %2d  cname %10s  nodes %2d  streams %d\n", 
      $i, $cname, 
      pdq::GetNodesCount(), pdq::GetStreamsCount();
}
...
for ($i = 0; $i < $no; $i++) {
   ...
   pdq::CreateClosed($wname, $pdq::TERM, $Nrdop, $Zrdop);
   $cname   = sprintf "%s%d", $L2C, $i;
...
if (WBACK) {
   ...
   for ($i = 0; $i < $no; $i++) {
      ...
      pdq::CreateClosed($wname, $pdq::TERM, $Nwbop, $Zwbop);
      $cname   = sprintf "%s%d", $L2C, $i;
   ...
   for ($i = 0; $i < $no; $i++) {
      $wname   = sprintf "%s%d", $WROP, $i;
      pdq::CreateClosed($wname, $pdq::TERM, $Nwthr, $Zwthr);
      $cname   = sprintf "%s%d", $L2C, $i;
      pdq::SetDemand($cname, $wname, $gen);
